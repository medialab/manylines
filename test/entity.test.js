...
describe('Concerning models', function() {
  ...
  it('should be possible to update items.', function(done) {
    ...
    test.entities.user.update(cache.userId, updateData, function(err, result) {
  ...
  it('should be possible to overwrite items.', function(done) {
    ...
    test.entities.user.set(cache.userId, userData, function(err, result) {
  ...
  it('should be possible to get an item.', function(done) {
    ...
    test.entities.user.get(cache.userId, function(err, result) {
  ...
  it('should be possible to get multiple items.', function(done) {
    ...
    test.entities.user.create({firstname: 'Michel', lastname: 'Ney'}, function(err, result) {

      // Getting multiple users
      test.entities.user.get([cache.userId, result.id], function(err, results) {
        assert.deepEqual(
          results,
          [
  ...
  it('should be possible to get an item with its identifier.', function(done) {
    ...
    test.entities.user.get(cache.userId, {id: true}, function(err, result) {
  ...
  it('should retrieve an inexistant item as null.', function(done) {
    ...
    test.entities.user.get('random', function(err, result) {
      assert(err === null);
      assert(result === null);
      done();
    });
  ...
  it('should be possible to remove an item.', function(done) {
    ...
    test.entities.user.remove(cache.userId, function(err) {
  ...
  it('should trigger an error when trying to remove an inexistant item.', function(done) {
    ...
    test.entities.user.remove('random', function(err) {
      assert(err.code === 13);
      done();
    });
  });
