/**
 * Manylines Entity Abstraction
 * =============================
 *
 * Entities aim at providing a high-level interface to Couchbase data so the
...
// Main class
function Entity(name, bucket, schema) {
  var self = this;
  ...
  // Validate against entity's schema
  this.check = function(data) {
    return types.check(data, this.schema);
  };
  ...
  this.create = function(data, callback) {
    if (!this.check(data))
      return callback(new Error('entity.create:wrong-data (' + this.name + ')'));
  ...
  this.update = function(id, data, callback) {
    ...
    this.get(id, function(err, result) {
      if (err)
        return callback(err);
      if (!result)
        return callback(new Error('entity.update:wrong-data'));
  ...
  this.set = function(id, data, callback) {
    if (!this.check(data))
      return callback(new Error('entity.set:wrong-data'));
  ...
  this.get = function(id, params, callback) {
    ...
    if (!types.check(id, 'string|array'))
      return callback(new Error('entity.get:wrong-parameters'));
