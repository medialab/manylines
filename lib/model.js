...
function Model(name, bucket, schema) {
  ...
  this.create = function(data, callback) {
    if (!this.check(data))
      return callback(new Error('model.create:wrong-data (' + this.name + ')'));
  ...
  this.update = function(id, data, callback) {
    ...
    this.get(id, function(err, result) {
      if (err)
        return callback(err);
      if (!result)
        return callback(new Error('model.update:wrong-data'));
  ...
  this.set = function(id, data, callback) {
    if (!this.check(data))
      return callback(new Error('model.set:wrong-data'));
  ...
  this.get = function(id, params, callback) {
    ...
    if (!types.check(id, 'string|array'))
      return callback(new Error('model.get:wrong-parameters'));
